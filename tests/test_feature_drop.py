...
def test_calc_corr_matrix():
    ...
    df = pd.DataFrame(np.repeat(correlated_cols, 1000, axis = 0), columns = ["First", "Second"])
...
def test_drop_high_corr_features():
    ...
    df = pd.DataFrame(np.repeat(correlated_cols, 1000, axis = 0), columns = ["First", "Second", "Third"])
